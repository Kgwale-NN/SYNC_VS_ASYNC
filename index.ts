...
function fetchDetails(userId:string,callback:(error:Error | null,details?:{name:string,email:string}) => void){
...
fetchUserId((error,userId) => {
    ...
    if(error){
        ...
        console.error("Error in fetching userId" , error.message)
        return
    }

    if(userId){

        fetchDetails(userId,(error,details) => {
            ...
            if(details){
             ...
             saveUserLog(details.name,details.email,(error,logStatus) =>{
                ...
                if(logStatus){
                    ...
                    console.console.log("All operations completed successfully");
